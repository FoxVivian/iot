...
const useSocket = () => {
  ...
  const fetchDataSensors = async () => {
    try {
      const response = await axios.get(`http://192.168.96.205:3000/realtime/api/data/data_sensors`);
  ...
  const fetchDataDevices = async () => {
    try {
      const response = await axios.get(`http://192.168.96.205:3000/realtime/api/data/data_devices`);
      setDataDevices(response.data); // Cập nhật state với dữ liệu nhận được
    } catch (error) {
      console.error("Error fetching data devices:", error);
    }
  ...
  const sendControlData = async (controlData) => {
    try {
      const response = await axios.post(`http://192.168.96.205:3000/realtime/api/control/device`, {...controlData, token});
      setDataDevices(response.data);
      console.log(response);
    } 
    catch (error) {
